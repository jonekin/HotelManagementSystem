            ...
            <div class="layui-card-body">
                <div class="store-cat-item"> <span><i class="layui-icon layui-icon-shop-fenlei"></i>类别：</span>
                    <ul id="getAllRoomType">
                        ...
                        <li data-id="0" <c:if test="${typeId == null}">class="active"</c:if>> <a class="fly-case-active" href="/house/list" data-type="toRoomTypeListByLists">全部</a></li>
                        <c:forEach var="roomType" items="${roomTypeList}">
                            <%-- 判断typeId房型ID值是否与房型列表中ID值相等，相等则选择该超链接，设置字体颜色为绿色 --%>
                            <li data-id="${roomType.id}" <c:if test="${typeId ==  roomType.id}">class="active"</c:if>> <a class="fly-case-active" href="/house/list/${roomType.id}" data-type="toRoomTypeListByLists">${roomType.typename}</a> </li>
                        </c:forEach>
                    </ul>
                </div>
